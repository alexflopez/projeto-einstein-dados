-- Criação da Camada Prata como Tabela Física Consolidada
USE opendatasus;
GO

-- Remove a tabela física caso já exista para recriar do zero com limpeza garantida
IF OBJECT_ID('dbo.prata_sindrome_gripal', 'U') IS NOT NULL
    DROP TABLE dbo.prata_sindrome_gripal;
GO
...
SELECT 
    ...
    CONVERT(VARCHAR(10), TRY_CAST(NULLIF(TRIM(dataNotificacao), 'NULL') AS DATE), 103) AS dataNotificacao,
    CONVERT(VARCHAR(10), TRY_CAST(NULLIF(TRIM(dataInicioSintomas), 'NULL') AS DATE), 103) AS dataInicioSintomas,
    CONVERT(VARCHAR(10), TRY_CAST(NULLIF(TRIM(dataEncerramento), 'NULL') AS DATE), 103) AS dataEncerramento,
    CONVERT(VARCHAR(10), TRY_CAST(NULLIF(TRIM(dataPrimeiraDose), 'NULL') AS DATE), 103) AS dataPrimeiraDose,
    CONVERT(VARCHAR(10), TRY_CAST(NULLIF(TRIM(dataSegundaDose), 'NULL') AS DATE), 103) AS dataSegundaDose,
    ...
    NULLIF(TRIM(codigoDosesVacina), 'NULL') AS codigoDosesVacina,

    -- Padronização dos códigos dos laboratórios
    CASE 
    ...
    END AS codigoLaboratorioSegundaDose,
    ...
    CONVERT(VARCHAR(10), TRY_CAST(NULLIF(TRIM(dataColetaTeste1), 'NULL') AS DATE), 103) AS dataColetaTeste1,
...
INTO dbo.prata_sindrome_gripal
